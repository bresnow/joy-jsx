Gun.chain.generateCert = async function(who, where, path) {
  ...
  var gun = user.back(-1);
  var pair = user._.sea;
  var certExists = who === "*" ? await user.get(path) : await user.get(path).get(who);
  ...
  if (who !== "*") {
    ...
    user && user.get(path).get(who).put(certificate).once((data) => {
      if (data) {
  ...
  user.get(path).put(certificate, ({ ok }) => {
    if (ok)
      console.log("Public Request Certificate created");
  });
};
Gun.chain.notify = function(pub, message) {
  var gun = this;
  gun.user(pub).get("certificates/notifications").once((cert) => {
    if (cert)
      gun.user(pub).get("notify").get("enabled").put(
        message,
        ({ ok, err }) => {
          if (ok) {
...
Gun.chain.sendNotification = async function(pub, message) {
  var user = this;
  var gun = user.back(-1);
  gun.get(pub).get("certificates/notifications").once((cert) => {
    if (cert)
      gun.get(pub).get("notifications").set(
        message,
        ({ ok, err }) => {
          if (ok)
            console.log("Notification sent to: ", pub);
          if (err)
...
Gun.chain.acceptConnection = async function(pub) {
  ...
  gun.get(pub).get("certificates/friends").get(user.is.pub).on(async (cert) => {
    console.log("Friend Cert: ", cert);
    if (cert) {
      if (!await gun.get(pub).get("friends").get(user.is.pub)) {
        gun.get(pub).get("friends").set(
          user.is.pub,
          ({ ok }) => {
...
Gun.chain.connect = async function(pub) {
  ...
  gun.get(`@${pub.substring(1)}`).get("notifications").set({ data: encrypted, epub: pair.epub });
  user.get("friends").set(pub);
};
...
Gun.chain.upload = function(b64, opt, cb) {
  ...
  async function splitAndUpload(b642, chunks) {
    chunks = chunks || 0;
    var b64String = b642.slice(0, opt.size);
    if (b642.length) {
      gun && gun.get(chunks).put(b64String, ({ ok, err }) => {
        if (ok) {
          chunks++;
          splitAndUpload(b642.slice(opt.size), chunks);
        } else {
          splitAndUpload(b642, chunks);
        }
      });
      cb((1 - b642.length / length) * 100);
    } else {
      cb(100);
      return this;
    }
